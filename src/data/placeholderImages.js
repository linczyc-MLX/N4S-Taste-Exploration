...
const categories = [
  'living_spaces', 'kitchens', 'primary_bedrooms', 'primary_bathrooms',
  'outdoor_living', 'exterior_architecture', 'exterior_landscape',
  'dining_spaces', 'libraries', 'materials_textures'
];

const subcategories = {
  living_spaces: ['great_room', 'formal_living', 'family_room'],
  kitchens: ['chef_kitchen', 'open_plan', 'galley'],
  primary_bedrooms: ['master_suite', 'sleeping_retreat'],
  primary_bathrooms: ['spa_suite', 'wet_room', 'traditional'],
  outdoor_living: ['terrace', 'pool_deck', 'loggia'],
  exterior_architecture: ['facade', 'entry', 'aerial'],
  exterior_landscape: ['arrival', 'pool', 'garden', 'entertaining'],
  dining_spaces: ['formal_dining', 'casual_dining'],
  libraries: ['traditional_library', 'modern_study'],
  materials_textures: ['stone', 'wood', 'metal', 'textile']
};
...
const generateImage = (id, category, subcategory, styleKey, styleValues) => {
  ...
  return {
    ...
    // Display label
    label: `${category.replace('_', ' ')} - ${subcategory.replace('_', ' ')}`,
    styleLabel: styleKey.replace(/_/g, ' ')
  };
};

// Generate the full placeholder library
let imageId = 1;
export const placeholderImages = [];

// Living Spaces - 20 images
const livingStyles = [
  ...
  { key: 'contemporary_cool_layered', ct: 2, ml: 7, wc: 7, og: 6, re: 4, complexity: 7, materials: ['marble', 'velvet', 'brass'], palette: 'cool_neutral', region: 'italian_modern' },
  ...
  { key: 'traditional_cool_minimal', ct: 7, ml: 3, wc: 6, og: 7, re: 3, complexity: 4, materials: ['limestone', 'linen', 'iron'], palette: 'greige', region: 'french_contemporary' },
  ...
  { key: 'mediterranean', ct: 6, ml: 6, wc: 3, og: 4, re: 5, complexity: 6, materials: ['terracotta', 'plaster', 'iron'], palette: 'warm_neutral', region: 'mediterranean' },
  { key: 'california', ct: 3, ml: 3, wc: 4, og: 4, re: 3, complexity: 4, materials: ['white_oak', 'glass', 'concrete'], palette: 'warm_neutral', region: 'california_modern' },
];

livingStyles.forEach((style, idx) => {
  const sub = subcategories.living_spaces[idx % 3];
  placeholderImages.push(generateImage(imageId++, 'living_spaces', sub, style.key, style));
});
livingStyles.slice(0, 10).forEach((style, idx) => {
  const sub = subcategories.living_spaces[(idx + 1) % 3];
  placeholderImages.push(generateImage(imageId++, 'living_spaces', sub, style.key, { ...style, complexity: style.complexity + 1 }));
});
...
const kitchenStyles = [
  ...
  { key: 'mediterranean', ct: 6, ml: 6, wc: 3, complexity: 6, materials: ['reclaimed_wood', 'tile', 'iron'], palette: 'earth_tones' },
];

kitchenStyles.forEach((style, idx) => {
...
bedroomStyles.forEach((style, idx) => {
...
bathroomStyles.forEach((style, idx) => {
...
const outdoorStyles = [
  ...
  { key: 'tropical', ct: 4, ml: 5, wc: 4, materials: ['teak', 'rattan', 'stone'], region: 'balinese' },
...
// Exterior Architecture - 20 images
const exteriorStyles = [
  ...
  { key: 'japanese', ct: 4, ml: 2, wc: 5, materials: ['wood', 'paper', 'stone'], region: 'japanese_modern' },
];

exteriorStyles.forEach((style) => {
